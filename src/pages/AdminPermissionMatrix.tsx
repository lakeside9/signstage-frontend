...
import type { PermissionMatrixRow, PlatformRole } from '../types';

const ROLE_COLUMNS: PlatformRole[] = ['PLATFORM_SUPPORT', 'PLATFORM_OPS', 'PLATFORM_SUPER'];
...
/**
 * 플랫폼 역할 × 권한키 매트릭스 관리 화면 — signstage-docs
 ...
 *
 * 메뉴(`MENU`)와 액션(`ACTION`) 권한이 같은 테이블·같은 토글 UI로 나온다(8장) — 무엇을
 * 감싸는지만 다를 뿐 메커니즘은 같다.
 */
export const AdminPermissionMatrix: FC = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const response = await api.get('/platform-admin/permissions');
        if (!cancelled) {
  ...
  const toggleAllowed = async (row: PermissionMatrixRow, roleValue: PlatformRole, nextAllowed: boolean) => {
    const cellKey = `${row.permissionDefinitionId}:${roleValue}`;
    setPendingKey(cellKey);
    try {
      await api.put(`/platform-admin/permissions/${row.permissionDefinitionId}`, {
        roleValue,
        allowed: nextAllowed,
      });
      setRows((prev) =>
