...
/**
 * 결과 PDF 위변조 검증 — signstage-docs business/ceremony-feature-migration-review.md
 * §8.13(2026-09-10). 화면 구성은 legacy(`~/Works/eform/source/signstage/signstage-frontend`
 * `VerifyAuthenticity.tsx`)를 참고했다 — 드래그앤드롭 업로드 영역, 파일 미리보기 카드, 결과
 * 카드, 하단 안내 박스 구성은 legacy와 같은 틀이다.
 *
 ...
 * /api/verification/documents`, 단건 응답). 그래서 결과 영역은 legacy처럼 서명자별 카드
 * 목록이 아니라 단일 결과 카드 하나다.
 *
 * <p>독립 전체화면 페이지가 아니라 `UserLayout`/`AdminLayout`의 콘텐츠 영역(사이드바+헤더가
 * 있는 `<Outlet/>`) 안에 뜬다(2026-09-14 사용자 요청) — 8.13절에서 의도했던 "로그인 없이도
 * 여는 완전 공개 페이지" 설계를 접고, 로그인한 사용자가 다른 화면들과 같은 틀 안에서 쓰는
 * 화면으로 바꿨다(`App.tsx`의 라우트도 최상위 공개 라우트에서 `/verify`(조직 사용자)와
 * `/admin/verify`(플랫폼 관리자) 두 보호 라우트로 옮겼다 — 같은 컴포넌트를 두 레이아웃에
 * 중복 등록한다).
 */
export const DocumentVerificationView: FC = () => {
  ...
  return (
    <div className="max-w-4xl space-y-6">
      <div>
        <h1 className="text-xl font-bold text-gray-950">진위여부 검증하기</h1>
        <p className="mt-1 text-sm text-gray-500">SignStage에서 생성된 결과 PDF가 맞는지, 서명 이후 변경되지 않았는지 확인합니다.</p>
      </div>
